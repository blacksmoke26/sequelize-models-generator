 ...
 * @example
 * ```typescript
 * import exportDbmlDiagram from './exportDbmlDiagram';
 *
 * // Export PostgreSQL schema to DBML file
 * await exportDbmlDiagram(
 *   'postgresql://user:password@localhost:5432/mydb',
 *   './output/schema.dbml'
 * );
 * ```
 */
export default async function exportDbmlDiagram(connectionString: string, outputFile: string): Promise<void> {
  try {
    ...
    renderOut('dbml-readme', `${path.dirname(outputFile)}/README.md`, {filename: path.basename(outputFile)})
