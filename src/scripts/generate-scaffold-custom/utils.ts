 ...
 */
const determineTsType = (columnInfo: ColumnInfo, modelName: string): string => {
 ...
 */
const addFieldComment = (vars: ModelTemplateVars, columnInfo: ColumnInfo): void => {
...
export const generateFields = (
  ...
  const tsType = determineTsType(columnInfo, modelName);

  addFieldComment(vars, columnInfo);

  const typeText = determineTypeText({
    isFK,
 ...
 */
const generateInterfaceDefinition = (columnInfo: ColumnInfo, interfacesVars: { text: string }): void => {
...
export const generateInterfaces = (columnInfo: ColumnInfo, vars: ModelTemplateVars, interfacesVars: { text: string }, dirname: string) => {
  ...
  generateInterfaceDefinition(columnInfo, interfacesVars);
...
export const generateEnums = (columnInfo: ColumnInfo, vars: ModelTemplateVars, modelName: string) => {
  ...
  vars.enums += generateEnumValues(values);
  vars.enums += sp(0, `}\n`);
};
...
export const generateOptions = (
  ...
  generateBasicOptions(modTplVars, options);
  generateTimestampOptions(modTplVars, options);
};
 ...
 */
const generateSingleIndex = (modTplVars: ModelTemplateVars, index: TableIndex): void => {
  if (index.comment) {
...
export const generateIndexes = (indexes: TableIndex[], modTplVars: ModelTemplateVars) => {
  ...
  for (const index of indexes) {
    generateSingleIndex(modTplVars, index);
  }
 ...
 */
const generateAttributeField = (columnInfo: ColumnInfo, modTplVars: ModelTemplateVars): void => {
 ...
 */
const generateAttributeReference = (foreignKey: ForeignKey | null, modTplVars: ModelTemplateVars): void => {
 ...
 */
const generateAttributeType = (columnInfo: ColumnInfo, modTplVars: ModelTemplateVars): void => {
 ...
 */
const generateAttributeFlags = (columnInfo: ColumnInfo, modTplVars: ModelTemplateVars): void => {
 ...
 */
const generateAttributeDefault = (columnInfo: ColumnInfo, modTplVars: ModelTemplateVars): void => {
...
export const generateAttributes = ({
  ...
  generateAttributeField(columnInfo, modTplVars);
  generateAttributeReference(foreignKey, modTplVars);
  generateAttributeType(columnInfo, modTplVars);
  generateAttributeFlags(columnInfo, modTplVars);

  if (columnInfo.comment) {
    modTplVars.attributes += sp(6, `comment: '%s',\n`, columnInfo.comment);
  }

  generateAttributeDefault(columnInfo, modTplVars);
 ...
 */
const generateSingleModelImport = (
  modTplVars: ModelTemplateVars,
...
export const generateRelationsImports = (tableRelations: Relationship[], modTplVars: ModelTemplateVars) => {
  ...
  for (const tableRelation of tableRelations) {
    generateSingleModelImport(modTplVars, tableRelation, imported);
  }
};
