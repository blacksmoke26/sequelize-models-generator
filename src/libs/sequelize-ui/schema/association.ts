...
export function associationsAreSame(args: AssociationsAreSameArgs): boolean {
  const {associationA, targetNameA, associationB, targetNameB} = args;
  return StringHelper.namesEqSingular(
...
function normalizeAssociationName(association: Association, targetName: string): string {
  return association.alias ? StringHelper.normalize(association.alias) : StringHelper.normalize(targetName);
}
